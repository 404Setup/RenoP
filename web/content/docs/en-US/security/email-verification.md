...
**Other login emails** lists private secondary addresses usable for password or Passkey login, with the existing
second-factor policy. They cannot reset a password or recover an account. Notifications and email password recovery use
the current primary email only. Each account can retain up to 128 addresses, including its primary email.
