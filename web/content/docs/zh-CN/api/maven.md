...
包页面为管理员和 L3/L4 协作者提供管理发布域团队入口，打开现有发布域团队页。成员变更适用于该域下所有包，包括旧仓库导入的包。长坐标与操作按钮分别换行，制品和最新
POM 信息使用完整内容宽度，依赖格式支持动画切换与键盘操作。
...
所有锁定 API 都支持 `reason: custom`，并要求非空的 `reason_text`，最多 256 个字符。可查看资源的用户可以看到这些纯文本，内容不得包含控制字符。使用预置原因时省略
`reason_text`。
