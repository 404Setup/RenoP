...
数据库读取缓存会合并并发远程查询和缓存未命中。远程 I/O 在分片锁之外执行；缓存失效后，新请求不会加入旧的加载任务，旧结果也不能重新填充缓存。凭据撤销仍依靠有界的本地失效记录。

索引负缓存和原生索引解析记录也使用所选缓存后端。文件与目录归属、待发布隔离标记及私有内容引用仍以本地状态为准。缓存故障只会导致未命中，不能使已安装文件消失或暴露待发布内容。

缓存键和条目元数据采用单独的有界预算，最低为 4 KiB；空值和长路径不能绕过淘汰机制。
