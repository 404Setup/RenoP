...
私钥不回显。服务商与站点密钥不变时留空可保留私钥。更改其中任意一项（包括关闭）会清除旧私钥。reCAPTCHA v3 默认最低分数为
0.5。在服务器域名与服务商控制台中允许实际站点域名。Friendly Captcha 可选全球或欧盟接口。

reCAPTCHA 和 Turnstile 会校验主机名。hCaptcha 的主机名字段仅用于统计，因此通过预期站点密钥校验。Friendly Captcha
校验站点密钥，并在服务商返回来源时校验来源。
