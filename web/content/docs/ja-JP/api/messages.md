...
`session_id` にブラウザーセッションの公開 ID を指定し、宛先を一人にして `all: false` を設定します。ID が空の場合は、そのアカウントのすべての認証情報から参照できます。サーバーはセッションの有効性と所有者を再確認し、失効済みまたは期限切れの宛先には `409` を返します。

`GET /api/messages/admin/sessions?username=alice&cursor=...` は有効なセッションを最大 100 件ずつ返します。残りは `next_cursor` で取得します。応答には公開 ID、端末情報、時刻を含み、セッションシークレットは含みません。管理権限が必要で、API トークンには `admin:notifications` も必要です。

一覧、未読数、個別・一括の既読化と削除は認証済みセッションの範囲に限定されます。他のセッションや API トークンでは、通知 ID が既知でも、対象 Cookie を併送しても対象通知を取得・変更できません。対象通知はメール配信から除外されます。宛先が一人の場合、画面はアニメーション付きの独自選択欄を表示し、初期値は全セッションです。
