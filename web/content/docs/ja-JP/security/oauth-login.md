...
RenoP は安定した subject をプロバイダーの種類、クライアント、エンドポイント、検証済み発行者に結び付けます。認可元の変更で既存の関連付けは移動しません。ブラウザーログインのアクセス・更新トークンは秘密の
`mfa_encryption_key` で当該セッション専用に暗号化され、セッションとともに削除されます。公開セッション、API
応答、ログには含まれません。再起動時もこの鍵を保持してください。保護された登録用アバターの一時暗号化トークンは、従来どおり確認または期限切れまで保持します。
...
GitHub アプリトークン、Google、設定済みインスタンスを含む GitLab、Stack Exchange
はログアウト時の取り消しに対応します。カスタムクライアント、Cloudflare、Microsoft は認可サービスが RFC 7009
エンドポイントを公開している場合にのみ `revocation_url` を設定します。URL
の推測やアカウント全体に対する管理者サインアウトは行いません。単一トークンでも認可全体を取り消すプロバイダーがあります。本機能導入前のセッションでは、取り消し可能な認可を得るために再ログインが必要です。

公開プロバイダー一覧と非公開の接続状態はバイナリ protobuf を使います。統合設定 API は `OAuthSettings` を使い、書き込みには
`replace_providers: true` が必要です。署名付きコールバックの形式は[認証 API](../api/authentication.md)を参照してください。
