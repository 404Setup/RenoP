...
Оба входных HTML-файла содержат только метаданные и точку монтирования. `js/shell.js` создаёт навигацию, страницы и
диалоги из компонентов `js/views/` общими DOM-функциями. Во frontend `js/bootstrap.js` сначала создаёт элементы, затем
загружает модули поведения, которым нужны готовые контролы при инициализации. Оформление передаётся через экранированные
HTML data-атрибуты и записывается как текст или свойства DOM. Измените JS-компонент и выполните
`pnpm run build:frontend` или `pnpm run build:web`; представления включены в проверку i18n. Сохранённые административные
вкладки применяются только к `/`, сохраняя прямые ссылки.
