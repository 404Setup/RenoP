...
Endpoints состояния со схемой возвращают бинарный protobuf. `Accept: application/json` не меняет формат. Сохраните ответ
в файл и декодируйте его как `InstanceStatus` из соответствующей схемы. Примеры показывают логические значения; текст
проверки здоровья и профили сохраняют свои форматы.
