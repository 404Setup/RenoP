...
## Уведомления для отдельного сеанса

Задайте в `session_id` публичный ID сеанса браузера, укажите одного получателя и `all: false`. Пустой ID сохраняет доступ для всех учётных данных аккаунта. Сервер повторно проверяет активность сеанса и его владельца; отозванная или истёкшая цель возвращает `409`.

`GET /api/messages/admin/sessions?username=alice&cursor=...` возвращает до 100 активных сеансов на страницу. Для следующих страниц используйте `next_cursor`. Ответ содержит публичные ID, сведения об устройстве и время, но не секреты сеансов. Нужны права менеджера; API-токену также требуется `admin:notifications`.

Списки, счётчики непрочитанных, индивидуальные и групповые операции чтения/удаления ограничены текущим аутентифицированным сеансом. Другие сеансы и API-токены не могут получить или изменить адресное уведомление, даже зная его ID или передав целевой cookie вместе с API-запросом. Такие уведомления не пересылаются по почте. Для одного получателя интерфейс показывает анимированный собственный список выбора, по умолчанию — все сеансы.
