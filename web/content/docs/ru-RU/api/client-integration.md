...
показывают декодированные поля, а не JSON-формат передачи. Endpoints только с JSON, родные протоколы пакетов, бинарные
части загрузок, текст проверки здоровья и отдельные ошибки сохраняют объявленные форматы.
