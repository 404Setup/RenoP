...
Корень задаётся `storage_path` в системных настройках, по умолчанию `storage`.
...
Общие данные S3 хранятся в закрытом пространстве `.renop-content-v1`. Включайте его в резервную копию вместе с объектами
репозиториев и сохраняйте закрытый индекс, чтобы уменьшить чтение метаданных после перезапуска. Индекс использует
версионированный поток записей JSON; старые снимки остаются читаемыми. Поведение дедупликации и восстановления описано
в [настройке репозиториев](/docs/configuration/repositories).
