...
Le **Groupe de permissions par défaut** définit les droits système et dépôt accordés à la création du compte, y compris
via un fournisseur tiers. La valeur par défaut est `default_permissions: [base]`. Les comptes existants restent
inchangés. Les anciens clients qui omettent `default_permissions` conservent les droits configurés.
