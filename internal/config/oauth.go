...
func (p OAuthProviderConfig) Resolved() OAuthProviderConfig {
	...
	case "github":
		...
		p.Claims = OAuthClaims{Subject: "id", Username: "login", Name: "name", Email: "email", Avatar: "avatar_url"}
		p.DisablePKCE = true
	...
	case "google":
		p.RevocationURL = "https://oauth2.googleapis.com/revoke"
	...
	case "gitlab":
		...
		p.RevocationURL = base + "/oauth/revoke"
...
func (p OAuthProviderConfig) Validate() error {
	...
	for _, value := range []string{p.Name, p.ClientID, p.ClientSecret, p.APIKey, p.Scopes, p.RevocationSecret} {
	...
	if p.RevocationSecret != "" && len(p.RevocationSecret) < 32 {
		return invalid
	}
	...
	for _, value := range []string{p.AuthorizeURL, p.TokenURL, p.UserInfoURL, p.JWKSURL, p.RevocationURL} {
