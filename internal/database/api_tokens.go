...
func (db *DB) GetAPITokenByHash(secretHash, username string) (*core.APITokenCredential, error) {
	...
	query := `SELECT api.id, api.name, api.scopes_json, api.created_at, api.expires_at,
		...
		account.expires_at, account.permissions_json, account.ban_reason, account.ban_reason_code, account.banned_at, account.banned_until,
	...
	var accountName, accountType, encryptedSecret, passwordHash, tokensJSON, createdAt, description, permissionsJSON, banReason, banReasonCode string
	...
	if err := row.Scan(&token.ID, &token.Name, &scopesJSON, &token.CreatedAt, &tokenExpiresAt,
		...
		&createdAt, &description, &accountExpiresAt, &permissionsJSON, &banReason, &banReasonCode, &bannedAt, &bannedUntil,
	...
	account, err := parseTokenRow(accountName, accountType, typeValue, encryptedSecret, passwordHash,
		tokensJSON, createdAt, description, accountExpiresAt, permissionsJSON, banReason, banReasonCode, bannedAt, bannedUntil,
