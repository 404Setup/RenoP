...
func TestGitHubOAuthExistingAccountAndSingleUseSession(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, "/packages", secondFactorURL.Path)
	require.Equal(t, "success", secondFactorURL.Query().Get("github_oauth"))
	require.NoError(t, secondFactor.Body.Close())
	sessions, err = db.ListUserSessions(identity.Username, "")
	require.NoError(t, err)
	require.Len(t, sessions, 2, "OAuth login must issue a session directly bypassing 2FA")
