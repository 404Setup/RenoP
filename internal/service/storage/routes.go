...
func SetupRoutes(app fiber.Router, state *core.AppState) {
	BindS3Index(state.Inner.FileIndex)
...
func HandleRepository(c fiber.Ctx, state *core.AppState) error {
	...
	user := auth.GetUser(c)
	engine, supported := repositoryEngines[repo.NormalizedFormat()]
	if !supported {
		return c.Status(fiber.StatusNotFound).SendString("Unsupported repository engine")
	}
	isRead := isRepositoryRead(c)
	if engine.prepare != nil {
		var handled bool
		var err error
		path, handled, err = engine.prepare(c, state, repo, cfg.StoragePath, path)
		if handled || err != nil {
			return err
		}
	}
	...
	if isRead {
		...
		canRead, err := engine.read(state, user, repo, sanitized, isRoot)
		if err != nil {
			return c.Status(fiber.StatusServiceUnavailable).SendString("Repository metadata is unavailable")
		}
		if !canRead {
			if engine.deniedRead != nil {
				if handled, err := engine.deniedRead(c, user, repo, sanitized); handled {
					return err
				}
			}
	...
	} else if engine.authorizeWrite != nil {
		if handled, err := engine.authorizeWrite(c, state, user, repo, sanitized); handled {
			return err
		}
	}
	...
	if engine.handle != nil {
		if handled, err := engine.handle(c, state, repo, cfg.StoragePath, path, isConcreteArtifact); handled {
			return err
		}
	}
