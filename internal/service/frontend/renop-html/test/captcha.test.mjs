...
test('CAPTCHA transport retries only a same-origin pre-mutation denial', async () => {
    ...
    const context = vm.createContext({
        ...
        fetch: async (url, options) => {
            ...
            return new Response('', {
                status: calls.length === 1 ? status : 201, headers: {
    ...
    context.countChallenge = () => {
        challenges++;
    };
    let result = await context.captchaFetch('/create', {
        method: 'POST',
        body: 'payload',
...
test('widget adapters initialize once, use compact controls, and return nonce-bound completions', async () => {
    ...
    for (const provider of ['recaptcha_v2', 'recaptcha_invisible', 'recaptcha_v3', 'turnstile', 'hcaptcha', 'friendlycaptcha']) {
        ...
        const widget = {
            dataset: {}, addEventListener: (event, fn) => {
                handlers[event] = fn;
            }
        };
        const parent = {postMessage: value => messages.push(value)};
        const window = {
            innerWidth: 300, addEventListener: (_, fn) => {
                onMessage = fn;
            }
        };
        const complete = () => rendered.callback('provider-response');
        window.hcaptcha = window.turnstile = {
            render: (_, options) => {
                rendered = options;
                complete();
            }
        ...
        window.grecaptcha = {
            render: (_, options) => {
                rendered = options;
                if (options.size !== 'invisible') complete();
                return 3;
            },
            ready: fn => fn(),
            execute: (key, options) => {
                if (options) return Promise.resolve('provider-response');
                complete();
            },
        };
        const context = vm.createContext({
            window, parent, location: {origin: 'https://example.test'},
            ...
            document: {
                documentElement: {}, body: {style: {}}, getElementById: () => widget,
                createElement: () => ({}), head: {
                    appendChild: script => {
                        scriptCount++;
                        if (script.type === 'module') {
                            script.onload();
                            handlers['frc:widget.complete']({detail: {response: 'provider-response'}});
        ...
        const event = {
            source: parent, origin: 'https://example.test', data: {
                kind: 'renop-captcha-init',
                provider,
                nonce: 'nonce',
                site_key: 'site',
                action: 'renop_registration',
                language: 'en-US',
                theme: 'light'
            }
        };
