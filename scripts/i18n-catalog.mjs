...
import {generateLocaleAssets} from './i18n-protobuf.mjs';
 ...
 */
async function loadLocaleFragment(i18nDir, locale, fragment, flat = false) {
    const fragmentPath = flat ? join(i18nDir, locale + '.js') : join(i18nDir, locale, fragment);
...
export async function scanI18nCatalog({
    ...
    for (const entry of readdirSync(i18nDir, {withFileTypes: true})) {
        ...
        const locale = flat ? entry.name.replace(/\.js$/, '') : entry.name;
        if ((flat ? !entry.isFile() || !entry.name.endsWith('.js') : !entry.isDirectory()) || !localePattern.test(locale)) {
    ...
    for (const locale of locales) {
        const fragments = flat ? ['common.js'] : listLocaleFragments(i18nDir, locale);
        fragmentNamesByLocale.set(locale, fragments);
        for (const fragment of fragments) loadTasks.push(loadLocaleFragment(i18nDir, locale, fragment, flat));
    }
...
/** Validate source dictionaries before writing the shared key index and language assets. */
export async function generateI18nCatalog({
    ...
    const scan = await scanI18nCatalog({i18nDir, referenceLocale, catalogName, sourceRoots, flat});
    console.log(`i18n validation OK: ${scan.locales.length} locales, ${scan.referenceFragments.length} fragments, ${scan.keyCount} English keys, ${scan.referenceCount} static references (${scan.durationMs.toFixed(1)} ms)`);
    const catalogs = new Map();
    for (const [locale, fragments] of scan.fragmentsByLocale) {
        const merged = Object.create(null);
        for (const fragment of fragments.values()) Object.assign(merged, fragment.translations);
        catalogs.set(locale, merged);
    }
    const assets = generateLocaleAssets(catalogs, catalogFile);
    for (const entry of readdirSync(i18nDir, {withFileTypes: true})) {
        if (entry.isFile() && generatedLocaleCatalogPattern.test(entry.name)) rmSync(join(i18nDir, entry.name));
    }
    return assets;
}
